...
# 1.0.7
플러그인에 커맨드의 등록을 여러개 가능하게 하기?
앱커맨드에 등록시키기?

# 1.0.6
desciption search가능
기존에는 name(약어) 를 기반으로 검색하고 실행하는 로직에서
data에 고유id를 도입하여, 중복된 name도 검색되도록 변경
기존에는 중복된 name을 검색하는 경우, 첫번째 요소만 검색되는 문제가 있었음

# 1.0.5 
설정에서 새로운 snippet이나 command를 등록할때,
name에 의해 자동정렬되도록 변경하여
다른 단축어와 비교가 용이하도록 개선
